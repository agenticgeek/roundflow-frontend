...
export function SetupWizardFooter({
  ...
  const resolvedContinueLabel =
    continueLabel ?? (isLastStep ? footer.launch : footer.continue)
  const showSkipStep = Boolean(onSkipStep) && !isLastStep
      ...
      {showSkipStep ? (
        <div className="mt-1.5 flex justify-center">
          <button
            ...
            onClick={onSkipStep}
            disabled={loading}
            className="text-sm font-medium text-muted transition-colors hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
          >
            {footer.skipStep}
          </button>
        </div>
